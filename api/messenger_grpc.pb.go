...
const (
	...
	Messenger_Subscribe_FullMethodName   = "/messenger.Messenger/Subscribe"
)
...
func (c *messengerClient) Subscribe(ctx context.Context, in *SubscribeRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ChatMessage], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
...
type Messenger_SubscribeClient = grpc.ServerStreamingClient[ChatMessage]
...
func (UnimplementedMessengerServer) Subscribe(*SubscribeRequest, grpc.ServerStreamingServer[ChatMessage]) error {
...
func _Messenger_Subscribe_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(MessengerServer).Subscribe(m, &grpc.GenericServerStream[SubscribeRequest, ChatMessage]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Messenger_SubscribeServer = grpc.ServerStreamingServer[ChatMessage]
